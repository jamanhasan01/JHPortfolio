  ...
  return (
    <section className="bg-black/10">
      <div className="container mx-auto px-4 py-8">
        <Title title={'Projects'} subTitle=""></Title>
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {projectsData.map((project) => (
            <div
              key={project.id}
              className="bg-slate-800 bg-opacity-10 relative flex flex-col justify-between items-center text-center border p-3 border-gray-600  w-full"
            >
              <img
                className="w-full mb-2 border border-gray-500 rounded-lg"
                src={project.image}
                alt={project.title}
              />
